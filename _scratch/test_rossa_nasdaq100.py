"""
Year-by-year Rossa algorithm analysis for NASDAQ 100 (2001-present).
...
import rossa
from datamarshal import load_nasdaq100_constituents
...
# OUTPUT DIRECTORIES
OUTPUT_DIR = Path(__file__).parent / "rossa_analysis_nasdaq100"
...
def analyze_year(
    ...
    try:
        # Get NASDAQ 100 constituents as of end of year
        tickers_date = analysis_end_date.strftime("%Y-%m-%d")
        tickers = load_nasdaq100_constituents(tickers_date)
        logger.info(f"Loaded {len(tickers)} NASDAQ 100 constituents")
...
def plot_slope_over_time(
    ...
    axes[0].set_title("Regression Slope Over Time (NASDAQ 100)", fontsize=12)
...
def main():
    ...
    logger.info("Starting year-by-year Rossa analysis for NASDAQ 100 (2001-present)")
    ...
    logger.info("\n" + "="*70)
    logger.info("SUMMARY STATISTICS (NASDAQ 100)")
    logger.info("="*70)
