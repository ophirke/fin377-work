"""
Year-by-year Rossa algorithm analysis for bottom N stocks of S&P 500 (2001-present).
...
# PARAMETERS
N_STOCKS = 500  # Change this to analyze bottom N stocks
START_YEAR = 2001
END_YEAR = None  # None means current year
LOOKBACK_DAYS = 365
CORRELATION_THRESHOLD = 0.9  # Keep edges where |correlation| >= this threshold
...
# OUTPUT DIRECTORIES
OUTPUT_DIR = Path(__file__).parent / f"rossa_analysis_sp500_bottom{N_STOCKS}"
...
def analyze_year(
    ...
    """
    Run Rossa algorithm for a single year on bottom N stocks and relate coreness to returns.
    
    Args:
        year: Analysis year
        n_stocks: Number of smallest stocks to select from S&P 500
    ...
    try:
        ...
        # Create a mapping of ticker to its S&P 500 rank (1-based, where 1 is largest cap)
        ticker_to_rank = {ticker: i + 1 for i, ticker in enumerate(all_tickers)}
        
        # Select bottom N stocks (smallest by weight/market cap)
        # The constituents are already sorted by weight, so take the last N
        selected_tickers = list(all_tickers)[-n_stocks:]
        logger.info(f"Selected bottom {len(selected_tickers)} stocks from {len(all_tickers)} SP500 constituents")
        
        # Fetch price data for BOTH analysis and returns windows
        logger.info(f"Fetching price data for {len(selected_tickers)} tickers...")
        price_data = fetch_and_cache_stock_data(tuple(selected_tickers))
        ...
        # Add S&P 500 rank for each stock
...
def plot_yearly_scatter(
    ...
    """Plot scatter plot of coreness vs returns for a single year with stock labels, colored by S&P 500 rank."""
    if results is None or "error" in diagnostics:
        return
    
    fig, ax = plt.subplots(figsize=(14, 8))
    
    # Create colormap from green (rank 1) to purple (rank 500)
    ...
    ax.set_title(
        f"Coreness vs Forward Returns - {year} (Correlation-filtered)\\n"
    ...
    sm.set_array([])
    cbar = plt.colorbar(sm, ax=ax, label="S&P 500 Rank")
...
def plot_slope_over_time(
    ...
    axes[0].set_title(f"Regression Slope Over Time (S&P 500 Bottom {n_stocks}, Correlation-filtered)", fontsize=12)
...
def main():
    ...
    logger.info(f"Starting year-by-year Rossa analysis for S&P 500 Bottom {N_STOCKS} stocks ({START_YEAR}-present)")
    ...
    for year in range(START_YEAR, end_year + 1):
        results, diagnostics = analyze_year(year, n_stocks=N_STOCKS, lookback_days=LOOKBACK_DAYS)
    ...
    plot_slope_over_time(years, slopes, r_squared_values, p_values, N_STOCKS)
    ...
    logger.info("\n" + "="*70)
    logger.info(f"SUMMARY STATISTICS (S&P 500 Bottom {N_STOCKS})")
    logger.info("="*70)
